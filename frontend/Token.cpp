...
Token *Token::Number(char firstChar, Source *source)
{
    ...
    else tokenError(token, "Invalid number", source->lineNumber() - 1);
...
Token *Token::SpecialSymbol(char firstChar, Source *source)
{
    ...
    switch (firstChar)
    {
        ...
        case '^' : token->type = TokenType::CAROT;		break;
        ...
        {
        	token->type = TokenType::ERROR;
        	tokenError(token, "Invalid Token", source->lineNumber());
        }
    }
...
void Token::tokenError(Token *token, string message, int linNum)
{
